    ...
    for i in range(0, len(table_names), batch_size):
        ...
        union_queries = [
            ...
            SELECT 
              ...
              CAST(content_type STRING) AS content_type,
        ...
        SELECT DISTINCT tmdb_id, content_type, genres, title, synopsis, source_table
        FROM all_data
...
WITH ranked AS (
  ...
  WHERE tmdb_id IS NOT NULL
    AND REGEXP_CONTAINS(tmdb_id, '^[0-9]+$')  -- Solo incluir tmdb_id numéricos
)
SELECT tmdb_id, content_type, genres, title, synopsis
FROM ranked
WHERE rank = 1;
"""
