            ...
            for subdir in subdirs:
                ...
                for obj in response.get("Contents", []):
                    ...
                    if file_key.endswith(".jsonl"):
                        ...
                        table_id = f"{PROJECT_ID}.{DATASET_ID}.{table_name}"
                        logger.info(f"Submitting task to process {file_key} and insert into {table_id}")
                        futures.append(executor.submit(self.insert_jsonl_data, file_key, table_id))

            for future in as_completed(futures):
                try:
                    future.result()
                except Exception as e:
                    logger.exception(f"Error in concurrent execution: {e}")
...
if __name__ == "__main__":
    ...
    do_transfer.process_spaces_data()
